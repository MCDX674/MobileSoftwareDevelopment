...
data class Event(
    val title: String,
    val description: String? = null, // 可空，默认值为null
    val daypart: String,
...
fun main() {
    // 测试创建Event对象
    val event = Event("测试活动", "活动描述", "Morning", 30)
    println(event)
}
...
data class Event(
    ...
    val daypart: Daypart, // 类型改为枚举Daypart
    val durationInMinutes: Int
)

fun main() {
    val event = Event("测试活动", "活动描述", Daypart.MORNING, 30)
    println(event)
}
